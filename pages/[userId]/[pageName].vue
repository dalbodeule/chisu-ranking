...
<template>
  <div class="container mx-auto p-4">
    <div class="text-gray-700 flex flex-row gap-[10px] items-center">
      <img class="w-[48px] h-[48px] rounded-[48px] border border-gray-500 mr-[10px]" :src="chzzkProfile?.channelImageUrl" :alt="`${chzzkProfile?.channelName} 프로필 이미지`" >
      <NuxtLink :to="`/${chzzkProfile?.channelId}`" class="text-gray-700 hover:text-blue-500 text-2xl">{{ chzzkProfile?.channelName }}</NuxtLink>
    ...
    <SaveLoadButtons v-if="isEditor" :user-id="userId" :page-name="pageName" @submit="submitPage" @load="loadPage"/>
